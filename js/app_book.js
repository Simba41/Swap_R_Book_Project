...
export async function init() 
{
  ...
  const me = await window.api.me().catch(()=>null);
  ...
  topAvatar && topAvatar.addEventListener('click', e => 
  {
    e.preventDefault?.(); 
    if (ownerId) location.hash = `#/user?id=${ownerId}`;
  ...
  chatTop && chatTop.addEventListener('click', () => ownerId &&
  ...
  if (own)    own.textContent    = displayName(owner);
  if (dist)   dist.textContent   = '';

  const likeBtn  = document.getElementById('likeBtn');
  const likeStat = document.getElementById('likeStat');

  renderLike(book._id || id, book.likedBy?.includes(myId), book.likes || 0);

  likeBtn && likeBtn.addEventListener('click', async () => 
  {
    try 
    {
      const alreadyLiked = likeBtn.dataset.liked === '1';
      let data;
      if (alreadyLiked) 
      {
        data = await window.api.books.unlike(book._id || id);
      } else 
      {
        data = await window.api.books.like(book._id || id);
      }
      renderLike(book._id || id, data.liked, data.likes);
    } catch (e) 
    {
  ...
  function renderLike(bookId, liked, count) 
  {
    if (!likeBtn || !likeStat) 
      return;
    ...
    likeStat.textContent = `${count} like${count === 1 ? '' : 's'}`;
  }
