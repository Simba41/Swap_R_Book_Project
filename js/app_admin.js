export async function init()
{
  ...
  const adminApi = 
  {
    users: (p) => httpGet('/api/admin/users', p),
    books: (p) => httpGet('/api/admin/books', p),
    reports: (p) => httpGet('/api/admin/reports', p),
    changes: (p) => httpGet('/api/admin/changes', p),
    conversations: () => httpGet('/api/admin/conversations'),
    messages: (p) => httpGet('/api/admin/messages', p),
  };
  ...
  document.getElementById('bSearch')?.addEventListener('click', async () => 
  {
    ...
    const data = await adminApi.books({ q });
  ...
  document.getElementById('showReports')?.addEventListener('click', async () => 
  {
    ...
    feed.innerHTML = '<h2>Reports</h2><ul>' + data.items.map(
      r => `<li>${r.text} — status: ${r.status || (r.resolved ? 'resolved' : 'open')}</li>`
    ).join('') + '</ul>';
  });

  document.getElementById('showChanges')?.addEventListener('click', async () => 
  {
    const data = await adminApi.changes();
